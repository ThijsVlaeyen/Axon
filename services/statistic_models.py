...
def db_get_statistics():
    ...
    rawstats = conn.execute("""
            SELECT
                ...
                (SUM(CASE WHEN s.status_id = 1 THEN 1 ELSE 0 END) + LOG((SUM(CASE WHEN s.status_id IN (1, 2) THEN 1 ELSE 0 END) + 1)) * (SUM(CASE WHEN s.status_id IN (1, 2) THEN 1 ELSE 0 END) / (SUM(CASE WHEN s.status_id IN (1, 2) THEN 1 ELSE 0 END) + 2))) / (SUM(CASE WHEN s.status_id IN (1, 2) THEN 1 ELSE 0 END) + 1) AS "score"
            FROM song 
